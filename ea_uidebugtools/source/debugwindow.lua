...
function DebugWindow.Initialize()
	...
	local abfind = DebugWindow.AbilityFind
	local desc = "desc"
	local mesh = Mesh.Toggle
	local changefont = SetNamesAndTitlesFont
end

function DebugWindow.OnShown()
    if not bustedloaded then return end
    if Busted and BustedGUI and BustedGUI.ToggleMainWindow then
...
function DebugWindow.Update( timePassed )
    ...
    if (DebugWindow.lastMouseX ~= SystemData.MousePosition.x or DebugWindow.lastMouseY ~= SystemData.MousePosition.y) then
...
-----------------CHAT HISTORY-----------------
function DebugWindow.AddInputHistory(text)
  table_insert(DebugWindow.history, text)
end
...
function DebugWindow.TextSend()
  local text = towstring(TextEditBoxGetText(SystemData.ActiveWindow.name))
    if text == L"" then
...
function DebugWindow.TextSender()
  ...
  DebugWindow.AddInputHistory(text)
  DebugWindow.ScrollToBottom()
  TextEditBoxSetText("DebugWindowTextBox", L"")
end
...
function DebugWindow.GetEntries()
  ...
  for i = 0, numEntries - 1 do
    ...
    if filterId == 9 or filterId == 10 then
      table_insert(entry, text)
    else
      table_insert(entry, label .. text)
    end
  end

  local copyText = entry[1] or L""
  for i = 2, #entry do
    copyText = copyText .. L"\n" .. entry[i]
  end

  DebugWindow.copyText = copyText
...
function DebugWindow.PreventType()
    TextEditBoxSetText("DevPadCopyLog", DebugWindow.copyText or L"")
end
...
do
  ...
  function DebugWindow.tableConcat(list, sep, i, j, ...)
    local first_conv_idx, converters = 4, { sep, i, j, ... }
    local t = {}
    ...
    if conv_types[type(sep)] then
      first_conv_idx, sep, i, j = 1
    elseif conv_types[type(i)] then
      first_conv_idx, i, j = 2
    elseif conv_types[type(j)] then
      first_conv_idx, j = 3
    end
    ...
    return orig_table_concat(t, sep, 1, out - 1)
  end
end
