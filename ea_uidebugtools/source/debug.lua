...
--------------------- eve() handle--------------------
function EVENT(text)
  if text == nil then text = L"" end
  if type(text) == "string" then text = towstring(text) end
  TextLogAddEntry("UiLog", 11, text)
end
------------------------------------------------
...
local function term_print(text)
    pp(text)
end
...
function hw(text)
  ...
  if text == nil or text == "" then
    if showing then
      ...
      pp("You must specify a window to highlight. Use hw\"WindowName\"")
    end
    return
  end

  if WindowGetShowing(text) then
    HelpTips.SetFocusOnWindow(text)
    pp(L"Highlighting window " .. towstring(text))
  else
    WindowSetShowing(HelpTips.FOCUS_WINDOW_NAME, false)
    pp("Window not currently showing.")
  end
end

--------------------------------LOGDUMP------------------------------------
function logdump(name, ...)
  if not name or select('#', ...) == 0 then
    pp("Usage: logdump(\"logname\", arguments)")
    return
  end
...
for k, v in pairs(_G) do
  if type(v) == "function" then
    table.insert(registeredfunctionlist, k)
  end
end
...
do
  ...
  function assert(condition, message)
    ...
    return luaAssert(condition, message)
  end
end