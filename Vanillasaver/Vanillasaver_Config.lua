...
function Vanillasaver_Config.Slash(input)
	if (not GUI) then
		-- parameters: title text, settings table, callback-function
		-- note that you need to have a settings table!
		GUI = LibConfig("Vanillasaver v" .. tostring(Vanillasaver.Settings.Version), Vanillasaver.Settings, true, Vanillasaver_Config.SettingsChanged)
		
		GUI:AddTab("Info")
		local infoText
		infoText = GUI("label", "Vanillasaver can set a check for abilities that will be ineffective if the target is immune or when the target")
...
function Vanillasaver_Config.SettingsChanged()
	GUI:Hide()
	Vanillasaver.UpdateSettings()
end