...
local LabelSetTextAlign = LabelSetTextAlign
...
local InterfaceCore_GetScale = InterfaceCore.GetScale
...
function WSCT:ObjectIDAnimation(adat, objectID)
  ...
  -- MoveWindowToWorldObject(adat.name, objectID, 1.0)  -- Emissary Removed
...
function WSCT.UpdateAnimation(elapsed)
  ...
  -- #4 EARLY-OUT: nothing animating, do nothing
  if next(arrAniData[WSCT.FRAME1]) == nil
     and next(arrAniData[WSCT.FRAME2]) == nil
     and next(arrAniData[WSCT.FRAME3]) == nil
  then
    return
  end
  ...
  if WSCT._lastUpdate >= throttleTime and elapsed > 0 then
    WSCT._fps = floor(1/elapsed)
  end

  WSCT._lastUpdate = WSCT._lastUpdate + elapsed

  -- Throttle the time till next update of animations and reset of eventCount
  if WSCT._lastUpdate >= throttleTime then
    WSCT._lastUpdate = WSCT._lastUpdate - throttleTime

    if WSCT._fps >= WSCT._throttleIdealFPS
      or WSCT._fps >= WSCT._eventCount/2
    then
      WSCT._eventCount = 0
    end
    ...
    for k,time in pairs(WSCT._buffsPermanentUntilDispelledKeep) do
      if (time + WSCT._buffsPermanentUntilDispelledKeepDuration <= timeNow) then
        WSCT._buffsPermanentUntilDispelledKeep[k] = nil
      end
    end
  end
end
...
function WSCT:DoAnimation(adat, elapsed)
   ...
   if (adat.crit) then
     ...
     while (adat.lastupdate > speed) do
       ...
       WindowAddAnchor(adat.textname, arrAlign[2], adat.name, arrAlign[adat.align], adat.posX, -adat.posY)
       --update icon
       if adat.icon then WindowSetAlpha(adat.iconname, adat.alpha) end
...
function WSCT:CritAnimation(adat, speed)
  ...
  if elapsedTime < fadeInTime then
    local alpha = (elapsedTime / fadeInTime) * adat.alpha

    if adat.icon then
      WindowSetAlpha(adat.iconname, alpha)
    end

    if adat.critsize then
      local critsize = floor(
        adat.critsize - ((adat.critsize - adat.textsize) * (elapsedTime / WSCT_CRIT_FADEINTIME))
      )
  ...
  if adat.critsize then
    WindowSetScale(
      adat.textname,
      adat.uiscale * WSCT_CRIT_SIZE_PERCENT * (adat.textsize / WSCT_TEXTSIZE_BASE)
    )
    if adat.icon then
      WindowSetScale(
        adat.iconname,
        .8 * adat.uiscale * WSCT_CRIT_SIZE_PERCENT * (adat.textsize / WSCT_TEXTSIZE_BASE)
      )
    end
    adat.critsize = nil
  end
  ...
  if elapsedTime < (fadeInTime + holdTime) then
    WindowSetFontAlpha(adat.textname, adat.alpha)
    if adat.icon then
      WindowSetAlpha(adat.iconname, adat.alpha)
    end
    return
  end
  ...
  if elapsedTime < (fadeInTime + holdTime + fadeOutTime) then
    local alpha = (1 - ((elapsedTime - holdTime - fadeInTime) / fadeOutTime)) * adat.alpha
    WindowSetFontAlpha(adat.textname, alpha)
    if adat.icon then
      WindowSetAlpha(adat.iconname, alpha)
    end
...
function WSCT:GetNextAniObj(frame)
  ...
  adat.anim = nil
  adat.init = nil
  adat.uiscale = InterfaceCore_GetScale()
  return adat
end
