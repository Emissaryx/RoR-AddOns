...
RoR_SoR.Enable()
RoR_SoR.OnCombat()   -- apply current state now
end
...
--attackers:defenders:reserved_A:reserved_B:wait_A:wait_B
	...
	local rootWin = "SoR_"..Window_Name
if not DoesWindowExist(rootWin) or not WindowGetShowing(rootWin) then
    RoR_SoR.ZoneTimer[Window_Name] = ZoneLockTimer
    return
end
...
function RoR_SoR.SET_FORT(Input)
    ...
    -- create if missing
    if not DoesWindowExist(rootWin) then
        CreateWindowFromTemplate(rootWin, "RoR_SoR_FortTemplate", "Root")
        RoR_SoR.OpenZones[Window_Name] = 5
        RoR_SoR.Timers[Window_Name] = {}
        RoR_SoR.Fort[Window_Name] = {BO={},Owner={},Stage=0,Health=L""}
        RoR_SoR.Restack()
    end

    -- skip heavy UI if hidden
    if not WindowGetShowing(rootWin) then
        RoR_SoR.ZoneTimer[Window_Name] = ZoneLockTimer
        return
    end

    if DoesWindowExist("SoR_"..Window_Name) then
        -- (rest of your function exactly as you had it)
        WindowClearAnchors("SoR_"..Window_Name.."Banner")
        WindowAddAnchor("SoR_"..Window_Name.."Banner","top", "SoR_"..Window_Name, "top", 0,RoR_SoR.GetBanner())
...
function RoR_SoR.SET_BO(Input)
    ...
    -- create if missing
    if not DoesWindowExist(rootWin) then
        CreateWindowFromTemplate(rootWin, "RoR_SoR_RealmTemplate", "Root")
        local tier = GetCampaignZoneData(tonumber(Window_Name)).tierId
        RoR_SoR.OpenZones[Window_Name] = tonumber(tier)
        RoR_SoR.Timers[Window_Name]    = {}
        RoR_SoR.BO_States[Window_Name] = {}
        RoR_SoR.KEEP_States[Window_Name] = RoR_SoR.KEEP_States[Window_Name] or {}
        RoR_SoR.Restack()
    end

    -- skip heavy UI when hidden
    if not WindowGetShowing(rootWin) then
        RoR_SoR.ZoneTimer[Window_Name] = ZoneLockTimer
        return
    end

    if DoesWindowExist(rootWin) then
        RoR_SoR.Timers[Window_Name] = {}
        WindowSetScale(rootWin,WindowGetScale("RoR_SoR_Main_Window"))
        DynamicImageSetTextureSlice(rootWin.."BG", RoR_SoR.ParingPortrait[RoR_SoR.GetParing(Window_Name)])
        RoR_SoR.ZoneTimer[Window_Name] = ZoneLockTimer
        ...
        if Input.zoneId == 100 or Input.zoneId == 106 then
            Num_Bo = 3
            Lock_Counter = 1
        else
            Num_Bo = 4
		...
		for i=1,Num_Bo do
			...
			if BO_State == 4 then
				if RoR_SoR.BO_States[Window_Name][i] ~= 4 then
					WindowStartAlphaAnimation( rootWin.."BO"..i.."Flag", Window.AnimationType.LOOP, 1.0, 0.2, 0.5, false, 0.0, 0 ) --start the Burning BO pulse
					RoR_SoR.BO_States[Window_Name][i] = 4
				end
				RealmColor = RoR_SoR.CappingRealmColors[BO_Owner+1]
				WindowSetDimensions(rootWin.."BO"..i.."Flag",31,31)
			else
				RoR_SoR.BO_States[Window_Name][i] = nil
				RealmColor = RoR_SoR.RealmColors[BO_Owner+1]
				WindowStopAlphaAnimation(rootWin.."BO"..i.."Flag")
			end

			LabelSetTextColor(rootWin.."BO"..i.."TIMER",RealmColor.r,RealmColor.g,RealmColor.b)
		...
		if RoR_SoR.Settings.ShowBONames	then
			...
			LabelSetText(rootWin.."BO"..i.."TIMER",ShortBOName)
			LabelSetText(rootWin.."BO"..i.."TIMER_BG",ShortBOName)
			DynamicImageSetTexture( rootWin.."BO"..i.."Flag", RoR_SoR.GetFlag(BO_Owner,BO_State),31,31 )
			DynamicImageSetTexture( rootWin.."BO"..i.."FlagBG", RoR_SoR.GetFlag2(BO_Owner,BO_State),31,31 )
			...
			--	LabelSetText(rootWin.."BO"..i.."TIMER",towstring(SoR_BO_SPLIT_TEXT_STREAM[2+(4*i)]))
			end
		...
		--Zonelock
		if Lock_Counter == 4 then LabelSetText(rootWin.."LockedText",towstring(GetStringFromTable("Hardcoded", 1268)))end
			WindowSetShowing(rootWin.."LockTint",(Lock_Counter==4))
...
local Multip = Input.aaoMultiplier
	...
	local rootWin = "SoR_"..Window_Name
	    ...
	    -- biggest win: skip all UI when hidden or missing
    if not DoesWindowExist(rootWin) or not WindowGetShowing(rootWin) then
        RoR_SoR.ZoneTimer[Window_Name] = ZoneLockTimer
        return
    end
...
--check if Fort Data:
	if RoR_SoR.Forts[tonumber(Input.zoneId)] ~= nil then
		...
		if not WindowGetShowing("SoR_"..Window_Name) then
    RoR_SoR.ZoneTimer[Window_Name] = ZoneLockTimer
    return
end
...
function RoR_SoR.MainTooltip()
		...
		Tooltips.SetTooltipText( 3, 1, L"LMB: Toggle Show/Hide<br>RMB: Settings")
		Tooltips.Finalize()
		Tooltips.AnchorTooltip( Tooltips.ANCHOR_WINDOW_TOP )
	return
end
...
function RoR_SoR.TIMER_UPDATE(elapsedTime)
    -- skip UI updates in combat if HideCombat is on
    local inCombat = RoR_SoR.Settings.HideCombat and GameData.Player.inCombat
    local doUI = RoR_SoR.Settings.Enabled and not inCombat
    ...
    if RoR_SoR.Timers ~= nil then
        for k,v in pairs(RoR_SoR.Timers) do
            if v ~= nil then
                for a,b in pairs(v) do
                    ...
                    -- only update UI if doUI is true
                    if doUI and DoesWindowExist("SoR_"..k) then
                        if RoR_SoR.Fort[tostring(k)] == nil and RoR_SoR.City[tostring(k)] == nil then
                            local t = towstring(TimeUtils.FormatClock(RoR_SoR.Timers[tostring(k)][a]))
                            LabelSetText("SoR_"..k.."BO"..a.."TIMER", t)
                            LabelSetText("SoR_"..k.."BO"..a.."TIMER_BG", t)
                            if RoR_SoR.BO_States[k][a] ~= 4 then
                                local h = 31 - (tonumber(RoR_SoR.Timers[tostring(k)][a]) / 1.34)
                                if (h >= 0) and (RoR_SoR.Timers[tostring(k)][a] >= 0) then
                                    WindowSetDimensions("SoR_"..k.."BO"..a.."Flag", 31, h)
                                else
                                    WindowSetDimensions("SoR_"..k.."BO"..a.."Flag", 31, 31)
                                end
                            end
                        else
                            local t = towstring(TimeUtils.FormatClock(RoR_SoR.Timers[tostring(k)][a]))..L"<icon29979>"
                            LabelSetText("SoR_"..k.."_TIMER", t)
                            LabelSetText("SoR_"..k.."_TIMER_BG", t)
                        end
                    end
                end
            end

            -- background/banner only when doUI
            if doUI then
        ...
        -- Popper logic only when doUI
        if doUI then
            local windowName = SystemData.MouseOverWindow and SystemData.MouseOverWindow.name
    ...
    if RoR_SoR.Settings.OnlyActive == true then
        if RoR_SoR.ZoneNames[GameData.Player.zone] ~= nil and (RoR_SoR.OpenZones[tostring(RoR_SoR.ZoneNames[GameData.Player.zone][1])] ~= nil or RoR_SoR.OpenZones[tostring(RoR_SoR.ZoneNames[GameData.Player.zone][2])] ~= nil) then
            for k,_ in pairs(RoR_SoR.OpenZones) do
    ...
    if RoR_SoR.ZoneTimer ~= nil then
        for k,v in pairs(RoR_SoR.ZoneTimer) do
            if DoesWindowExist("SoR_"..k) then
                if v > 0 then
                    RoR_SoR.ZoneTimer[k] = v - elapsedTime
                else
                    RoR_SoR.RemoveWindow(k)
        ...
        if doUI then
            if RoR_SoR.SafeRestack then RoR_SoR.SafeRestack() else RoR_SoR.Restack() end
        end
    end

    if doUI and RoR_SoR.Pairings ~= nil then
        for k,_ in pairs(RoR_SoR.Pairings) do
            if DoesWindowExist("SoR_P_"..k) then
                ...
                if RoR_SoR.Pairings[k].Timer and RoR_SoR.Pairings[k].Timer > 0 then
                    ...
                    local t = TimeUtils.FormatTimeCondensed(RoR_SoR.Pairings[k].Timer)
                    LabelSetText("SoR_"..Window_Name.."_TIMER", t)
                    LabelSetText("SoR_"..Window_Name.."_TIMER_BG", t)
                end
                if RoR_SoR.Pairings[k].Timer2 and RoR_SoR.Pairings[k].Timer2 > 0 then
                    ...
                    local t2 = TimeUtils.FormatTimeCondensed(RoR_SoR.Pairings[k].Timer2)
                    LabelSetText("SoR_"..Window_Name.."Lock_STATUS", t2)
                    LabelSetText("SoR_"..Window_Name.."Lock_STATUS_BG", t2)
                end
            end
        end
    end

    for k,_ in pairs(RoR_SoR.SafeKeep) do
        local FillStart, FillEnd = 90, 360
        if doUI and DoesWindowExist("SoR_"..k) then
            if RoR_SoR.SafeKeep[k][1] > 0 and 600 > RoR_SoR.SafeKeep[k][1] then
                RoR_SoR.SafeKeep[k][1] = RoR_SoR.SafeKeep[k][1] - elapsedTime
                local t = TimeUtils.FormatTimeCondensed(RoR_SoR.SafeKeep[k][1])
                LabelSetText("SoR_"..k.."KEEP1SAFETIMER", t)
                LabelSetText("SoR_"..k.."KEEP1SAFETIMERBG", t)
                CircleImageSetFillParams("SoR_"..k.."KEEP1SAFECircle", FillStart, FillEnd - ((RoR_SoR.SafeKeep[k][1]/60) * 36))
            end
            if RoR_SoR.SafeKeep[k][2] > 0 and 600 > RoR_SoR.SafeKeep[k][2] then
                RoR_SoR.SafeKeep[k][2] = RoR_SoR.SafeKeep[k][2] - elapsedTime
                local t2 = TimeUtils.FormatTimeCondensed(RoR_SoR.SafeKeep[k][2])
                LabelSetText("SoR_"..k.."KEEP2SAFETIMER", t2)
                LabelSetText("SoR_"..k.."KEEP2SAFETIMERBG", t2)
                CircleImageSetFillParams("SoR_"..k.."KEEP2SAFECircle", FillStart, FillEnd - ((RoR_SoR.SafeKeep[k][2]/60) * 36))
            end
        end
    end
    ...
    -- Door% in ram (only when UI active)
    if doUI and GameData.SiegeWeapon.fireMode == GameData.SiegeFireMode.GOLF and GameData.SiegeWeapon.worldObjNum ~= 0 then
        local Zone
        ...
        if SiegeWeaponGolfFireWindow.KeepInfo[Zone] ~= nil and SiegeWeaponGolfFireWindow.KeepInfo[Zone][GameData.Player.realm] ~= nil then
            ...
            if CurrentKeepInfo ~= nil then
                ...
                if CurrentKeepInfo[State] ~= nil then
                    LabelSetText("RoR_SoR_DoorPercentageLabel", L"<icon29980>Door: "..towstring(CurrentKeepInfo[State])..L"%")
...
function RoR_SoR.SetWindowShow()
    local inCombat = RoR_SoR.Settings.HideCombat and (GameData.Player.inCombat == true)
    local showUI   = RoR_SoR.Settings.Enabled and not inCombat

    WindowSetShowing("RoR_SoR_ButtonCombat", inCombat)
    WindowSetShowing("RoR_SoR_Button", not inCombat)
    WindowSetShowing("RoR_SoR_Main_Window", showUI)
    WindowSetShowing("RoR_SoR_Popper", showUI)

    for zoneId,_ in pairs(RoR_SoR.OpenZones or {}) do
        local w = "SoR_"..tostring(zoneId)
        if DoesWindowExist(w) then
            WindowSetShowing(w, showUI)
        end
    end
end

function RoR_SoR.OnCombat()
  if not RoR_SoR.Settings.HideCombat then
    RoR_SoR.SetWindowShow()
    return
  end

  local inCombat = GameData.Player.inCombat == true

    -- Hide windows in combat if HideCombat is enabled
    WindowSetShowing("RoR_SoR_ButtonCombat", inCombat)
    WindowSetShowing("RoR_SoR_Main_Window", not inCombat)
    WindowSetShowing("RoR_SoR_Popper", not inCombat)

  for zoneId,_ in pairs(RoR_SoR.OpenZones or {}) do
    local w = "SoR_"..tostring(zoneId)
    if DoesWindowExist(w) then
      WindowSetShowing(w, not inCombat)
    end
  end
...
local function MakeCallBack( SelectedOption )
  ...
  EA_Window_ContextMenu.CreateContextMenu( SystemData.MouseOverWindow.name, EA_Window_ContextMenu.CONTEXT_MENU_1,L"SoR Settings")
  EA_Window_ContextMenu.AddMenuDivider( EA_Window_ContextMenu.CONTEXT_MENU_1 )
