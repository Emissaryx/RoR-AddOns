...
local WindowSetHandleInput = WindowSetHandleInput
...
local WindowGetOffsetFromParent = WindowGetOffsetFromParent
local WindowGetShowing = WindowGetShowing
local WindowSetParent = WindowSetParent
local WindowSetLayer = WindowSetLayer
...
local SliderBarSetCurrentPosition = SliderBarSetCurrentPosition
local SliderBarGetCurrentPosition = SliderBarGetCurrentPosition
...
local LogDisplayScrollToBottom = LogDisplayScrollToBottom
...
do
	function TidyChatCore.SetWindowGroup(wndGroupId,
		...
		if (wndGroup and wndGroup.used) then
			...
			if (chatwindow_background_showing == nil) then
				chatwindow_background_showing = Settings[wndGroupId]
						.chatwindow_background_showing
			end
			...
			local x, y = WindowGetOffsetFromParent(wndGroupName)
	...
	function TidyChatCore.SetWindowTextLog(tabManagerId,
		...
		if (tab and tab.used) then
			...
			WindowSetAlpha(logDisplayName .. "ToBottomButton",
				(scrollbar_position == c_SCROLLBAR_POSITION_HIDDEN and 0) or scrollbar_alpha)
			...
			WindowAddAnchor(logDisplayName, "bottomright", wndGroupName .. "Foreground", "bottomright", textLogAnchor2XOffset,
				0)
		end
	end
	...
	function TidyChatCore.SetTextEntry(textentry_channel_showing,
		...
		WindowAddAnchor(c_TEXT_ENTRY_WINDOW .. "EntryBox", "topleft", c_TEXT_ENTRY_WINDOW, "topleft", EntryBoxAnchorXOffset,
			0)
		...
		WindowAddAnchor(c_TEXT_ENTRY_WINDOW, TEAnchorPoint .. "left", TEAnchorRelativeWindow, TEAnchorRelativePoint .. "left",
			0, 0)
		WindowAddAnchor(c_TEXT_ENTRY_WINDOW, TEAnchorPoint .. "right", TEAnchorRelativeWindow,
			TEAnchorRelativePoint .. "right", 0, 0)
	end
end
-----------------------------------
-- end Core Frames
...
do
	function TidyChatCore.AddNewChannels(...)
		...
		for _, channelData in ipairs(newChannelsList) do
			...
			ChatSettings.ChannelColors[channelData.id] = ChatSettings.ChannelColors[channelData.id] or channelData
					.defaultColor
		end
	...
	function TidyChatCore.GetEntryCache(tabManagerId)
		...
		if (tab and tab.used) then
			...
			local logTable = {
				{ logName = "Chat",   index = 0 },
				{ logName = "Combat", index = 0 },
			}
				...
				for _, logData in ipairs(logTable) do
					if logData.index < logData.numEntries then
						if logData.entryNum ~= logData.index then
							...
							logData.timestamp, logData.filterId, logData.text =
									TextLogGetEntry(logData.logName, logData.index)

							if filtersTable[logData.filterId] == nil then
								filtersTable[logData.filterId] =
										LogDisplayGetFilterState(logDisplayName, logData.logName, logData.filterId)
							end
...
do
	...
	function TidyChatHooks.SetupHooks()
		...
		EA_ChatWindow.UpdateAllChannelColors      = TidyChatHooks.UpdateAllChannelColorsHook
	...
	function TidyChatHooks.StartAlphaAnimationHook(...)
		...
		for wndGroupTabId, tab in ipairs(wndGroup.Tabs) do
			...
			if (wndGroupTabId == activeTab) then
				...
				WindowStartAlphaAnimation(tabName .. "TextLogScrollbar", Window.AnimationType.SINGLE, startAlpha, endAlpha,
					duration, false, 0, 0)
				WindowStartAlphaAnimation(tabName .. "TextLogToBottomButton", Window.AnimationType.SINGLE, startAlpha, endAlpha,
					duration, false, 0, 0)
			else
				WindowSetAlpha(tabName .. "TextLogScrollbar", endAlpha)
				WindowSetAlpha(tabName .. "TextLogToBottomButton", endAlpha)
	...
	function TidyChatHooks.UpdateAllChannelColorsHook(...)
		TidyChatUtils.Debug("TidyChatHooks.UpdateAllChannelColorsHook")
...
do
	function TidyChatFrames.Initialize()
		...
		WindowRegisterCoreEventHandler(c_TEXT_ENTRY_WINDOW .. "ChannelButton", "OnMButtonUp",
		...
		WindowRegisterCoreEventHandler(c_TEXT_ENTRY_WINDOW .. "EntryBoxTextInput", "OnLButtonDown",
			"TidyChat.OnEntryBoxTextInputLBD")

		TidyChat.OnEntryBoxUpdateShowing()

		CreateWindow(c_TEXT_ENTRY_ANCHOR, false)
		LayoutEditor.RegisterWindow(c_TEXT_ENTRY_ANCHOR, L "Chat Text Entry", L "Where Text Entry are displayed.", true,
			false, false, nil)
		...
		if (DoesWindowExist(c_TEXT_ENTRY_WINDOW .. "EntryBoxLanguageButton") and WindowGetShowing(c_TEXT_ENTRY_WINDOW .. "EntryBoxLanguageButton")) then
			...
			WindowAddAnchor(c_TEXT_ENTRY_WINDOW .. "EntryBoxTextInput", "topright",
				c_TEXT_ENTRY_WINDOW .. "EntryBoxChannelLabel", "topleft", 2, -5)
			WindowAddAnchor(c_TEXT_ENTRY_WINDOW .. "EntryBoxTextInput", "bottomright", c_TEXT_ENTRY_WINDOW .. "EntryBox",
				"bottomright", -4, 2)
			...
			WindowAddAnchor(c_TEXT_ENTRY_WINDOW .. "EntryBoxBG", "bottomright", c_TEXT_ENTRY_WINDOW .. "EntryBox",
				"bottomright", 0, 0)
		end
	...
	function TidyChatFrames.InitializeChannelMenuTidyChannelButtons()
		local x, y = WindowGetDimensions(c_CHANNEL_MENU)
		...
		ButtonSetText(c_CHANNEL_MENU .. "AllianceButton", L "/" ..
			GetChatString(StringTables.Chat.CHAT_CHANNEL_NAME_ALLIANCE))
		...
		ButtonSetText(c_CHANNEL_MENU .. "ScenarioButton", L "/" ..
			GetChatString(StringTables.Chat.CHAT_CHANNEL_NAME_SCENARIO))
	...
	function TidyChatFrames.UpdateTidyChannelButtonsColors()
		...
		ButtonSetTextColor(channelButton, Button.ButtonState.PRESSED_HIGHLIGHTED, channelColor.r, channelColor.g,
			channelColor.b)
...
do
	...
	TidyChatLogs.NewLogFilters = {
		Auction          = ChatChannel(L "Auction", SystemData.ChatLogFilters.AUCTION, "Chat", true, DefaultColor
			.CLEAR_WHITE),

		AdvancedLootRoll = ChatChannel(L "Advanced Loot Rolls", c_ADVANCED_LOOT_ROLL_CHANNEL_ID, "Chat", false,
			ChannelColor(121, 92, 99)),
	}
...
--]]
	...
	-- Language - russian
	--[[
TEXT_YOU_SELECT_NEED_FOR	�� ���������� � ��������: <<1>>.
TEXT_YOU_SELECT_GREED_FOR	�� ������ �������: <<1>>.
TEXT_YOU_SELECT_PASS_FOR	�� ������������� �� ��������: <<1>>.
TEXT_SELECTS_NEED_FOR	<<C:1>> ��������� � ��������: <<2>>.
TEXT_SELECTS_GREED_FOR	<<C:1>> ����� �������: <<2>>.
TEXT_SELECTS_PASS_FOR	<<C:1>> ������������ �� ��������: <<2>>.
TEXT_NEED_ROLL_HEADER	<<C:1>> - ���������� ����� �����������.
TEXT_GREED_ROLL_HEADER	<<C:1>> - ���������� ����� ��������.
TEXT_ALL_PASSED_HEADER	<<C:1>> - ��� ����������.
TEXT_LOOT_TIE_BREAKER	�����! �������������� ����������...
TEXT_YOU_ROLL_NUM_BONUS	�� ������� ������: <<1>> (<<2>> + <<3>> ���.).
TEXT_NAME_ROLLS_NUM_BONUS	<<C:1>> ������� ������: <<2>> (<<3>> + <<4>> ���.).
TEXT_YOU_ROLL_NUMBER	�� ������� ������: <<1>>�.
TEXT_NAME_ROLLS_NUMBER	<<C:1>> ������� ������: <<2>>�.
TEXT_WINNER_HEADER	���������� - <<1>>.
--]]
	...
	-- Language - german
	--[[
TEXT_YOU_SELECT_NEED_FOR	Ihr w�hlt "Bedarf" f�r <<akk,A:1>>.
TEXT_YOU_SELECT_GREED_FOR	Ihr w�hlt "Gier" f�r <<akk,A:1>>.
TEXT_YOU_SELECT_PASS_FOR	Ihr w�hlt "Passen" f�r <<akk,A:1>>.
TEXT_SELECTS_NEED_FOR	<<C:1>> w�hlt "Bedarf" f�r <<akk,A:2>>.
TEXT_SELECTS_GREED_FOR	<<C:1>> w�hlt "Gier" f�r <<akk,A:2>>.
TEXT_SELECTS_PASS_FOR	<<C:1>> w�hlt "Passen" f�r <<akk,A:2>>.
TEXT_NEED_ROLL_HEADER	F�r <<akk,A:1>> wird der Bedarf ausgew�rfelt:
TEXT_GREED_ROLL_HEADER	F�r <<akk,A:1>> wird die Gier ausgew�rfelt:
TEXT_ALL_PASSED_HEADER	<<1>>: Alle haben gepasst.
TEXT_LOOT_TIE_BREAKER	Gleichstand! Gewinner wird ausgew�rfelt...
TEXT_YOU_ROLL_NUM_BONUS	Ihr w�rfelt eine <<1>> (<<2>> + <<3>> Punkte).
TEXT_NAME_ROLLS_NUM_BONUS	<<1>> w�rfelt eine <<2>> (<<3>> + <<4>> Punkte).
TEXT_YOU_ROLL_NUMBER	Ihr w�rfelt eine <<1>>.
TEXT_NAME_ROLLS_NUMBER	<<1>> w�rfelt eine <<2>>.
...
--]]
	...
	if (SystemData.Settings.Language.active == SystemData.Settings.Language.GERMAN) then
		find_TEXT_SELECT = L " w�hlt [\"BedarfGierPassen]+ f�r "
		find_TEXT_NEEDGREED_ROLL_HEADER = L "^F�r d[asen]+ .+ wird d[erie]+ ([BedarfGier]+) ausgew�rfelt:$"
		find_TEXT_ALL_PASSED_HEADER = L ": Alle haben gepasst%.$"
		equal_TEXT_LOOT_TIE_BREAKER = L "Gleichstand! Gewinner wird ausgew�rfelt..."
		match_TEXT_ROLL_NUMBER = L "^([^%s]+) w�rfelt eine ([0-9]+)%.$"
...
TEXT_YOU_SELECT_GREED_FOR	Vous avez choisi Cupidit� pour <<1>>.
TEXT_YOU_SELECT_PASS_FOR	Vous avez choisi de Passer pour <<1>>.
TEXT_SELECTS_NEED_FOR	<<1>> choisit Besoin pour <<2>>.
TEXT_SELECTS_GREED_FOR	<<1>> choisit Cupidit� pour <<2>>.
TEXT_SELECTS_PASS_FOR	<<1>> choisit de Passer pour <<2>>.
TEXT_NEED_ROLL_HEADER	<<1>> - Lancer requis.
TEXT_GREED_ROLL_HEADER	<<1>> - Jet de cupidit�.
TEXT_ALL_PASSED_HEADER	<<1>> - Tous pass�s.
TEXT_LOOT_TIE_BREAKER	Il y a une �galit�! Jet de d� pour d�partager...
...
--]]
	...
	if (SystemData.Settings.Language.active == SystemData.Settings.Language.FRENCH) then
		find_TEXT_SELECT = L " choisi[t]? [BesoinCupidit�de Passer]+ pour "
		find_TEXT_NEEDGREED_ROLL_HEADER = L " %- ([Lancer requisJet de cupidit�]+)%.$"
		find_TEXT_ALL_PASSED_HEADER = L " %- Tous pass�s%.$"
		equal_TEXT_LOOT_TIE_BREAKER = L "Il y a une �galit�! Jet de d� pour d�partager..."
...
--]]
	...
	if (SystemData.Settings.Language.active == SystemData.Settings.Language.ITALIAN) then
		find_TEXT_SELECT = L "[Ss]elezion[ia] [BisognoAvidit�Passaggio]+ per "
		find_TEXT_NEEDGREED_ROLL_HEADER = L " %- Tiro ([BisognoAvidit�]+)%.$"
		find_TEXT_ALL_PASSED_HEADER = L " %- Tutti passano%.$"
		equal_TEXT_LOOT_TIE_BREAKER = L "C'� stato un pareggio! Rilancia per uno spareggio..."
...
TEXT_YOU_ROLL_NUM_BONUS	Tiras<<1>> (Bonificaci�n de <<2>> + <<3>>).
TEXT_NAME_ROLLS_NUM_BONUS	<<1>> tira <<2>> (<<3>> + <<4>> bonificaci�n).
	...
	function TidyChatLogs.ProcessLootRollEntry(text)
		...
		if value1 and value2 then
			...
			if itemRollData then
				itemRollData.playersRollDataStr =
						(itemRollData.playersRollDataStr and
							(itemRollData.playersRollDataStr .. L "," .. value1 .. L " " .. value2))
						or (value1 .. L " " .. value2)
			end
		...
		if (wstring_find(text, find_TEXT_WINNER_HEADER)) then
			...
			TextLogAddEntry("Chat", c_ADVANCED_LOOT_ROLL_CHANNEL_ID,
				L "<LINK data=\"TCROLL:" ..
				usedCacheIndex ..
				L "\" text=\"" ..
				linkText .. L "\" color=\"" .. mfloor(color.r) .. L "," .. mfloor(color.g) .. L "," .. mfloor(color.b) .. L "\">")
...
do
	...
	function TidyChatOptions.Reset()
		...
		for index, groupTabName in ipairs(TidyChatOptions.GroupTabs) do
			ButtonSetPressedFlag(groupTabName .. TCHAT_WINDOWS_TABS_SHOWING_CHECKBOX .. "Button",
				Settings[index].chatwindow_tabs_showing)
			ButtonSetPressedFlag(groupTabName .. TCHAT_WINDOWS_BG_SHOWING_CHECKBOX .. "Button",
				Settings[index].chatwindow_background_showing)
			ButtonSetPressedFlag(groupTabName .. TCHAT_WINDOWS_CLICK_THROUGH_CHECKBOX .. "Button",
				Settings[index].chatwindow_click_through)
			ComboBoxSetSelectedMenuItem(groupTabName .. TCHAT_WINDOWS_SCROLLBAR_POSITION_COMBO,
				Settings[index].scrollbar_position)
			SliderBarSetCurrentPosition(groupTabName .. TCHAT_WINDOWS_SCROLLBAR_FADEIN_ALPHA_SLIDER,
				Settings[index].scrollbar_fadein_alpha)
			ButtonSetPressedFlag(groupTabName .. TCHAT_WINDOWS_SCROLLBAR_FADEOUT_CHECKBOX .. "Button",
				Settings[index].scrollbar_fadeout_enable)
			SliderBarSetCurrentPosition(groupTabName .. TCHAT_WINDOWS_SCROLLBAR_FADEOUT_ALPHA_SLIDER,
				Settings[index].scrollbar_fadeout_alpha)
		end
	...
	function TidyChatOptions.OnApply()
		...
		for index, groupTabName in ipairs(TidyChatOptions.GroupTabs) do
			...
			SavedSettings[index].chatwindow_tabs_showing       = ButtonGetPressedFlag(groupTabName ..
				TCHAT_WINDOWS_TABS_SHOWING_CHECKBOX .. "Button")
			SavedSettings[index].chatwindow_background_showing = ButtonGetPressedFlag(groupTabName ..
				TCHAT_WINDOWS_BG_SHOWING_CHECKBOX .. "Button")
			SavedSettings[index].chatwindow_click_through      = ButtonGetPressedFlag(groupTabName ..
				TCHAT_WINDOWS_CLICK_THROUGH_CHECKBOX .. "Button")
			SavedSettings[index].scrollbar_position            = ComboBoxGetSelectedMenuItem(groupTabName ..
				TCHAT_WINDOWS_SCROLLBAR_POSITION_COMBO)
			SavedSettings[index].scrollbar_fadein_alpha        = SliderBarGetCurrentPosition(groupTabName ..
				TCHAT_WINDOWS_SCROLLBAR_FADEIN_ALPHA_SLIDER)
			SavedSettings[index].scrollbar_fadeout_enable      = ButtonGetPressedFlag(groupTabName ..
				TCHAT_WINDOWS_SCROLLBAR_FADEOUT_CHECKBOX .. "Button")
			SavedSettings[index].scrollbar_fadeout_alpha       = SliderBarGetCurrentPosition(groupTabName ..
				TCHAT_WINDOWS_SCROLLBAR_FADEOUT_ALPHA_SLIDER)
		end
...
function TidyChatOptions.OnRadioLBU()
	local radioName = SystemData.MouseOverWindow.name
...
do
	...
	local function GetEntriesForCopy(firstEntry)
		...
		LabelSetText(c_TIDY_CHAT_COPY .. "EntryCount",
			L "Entries " .. firstEntry .. L " to " .. lastEntry .. L " of " .. #entryCache .. L " entries")
