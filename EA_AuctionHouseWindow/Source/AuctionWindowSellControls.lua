...
function AuctionWindowSellControls.CalculateDeposit()
    ...
    --d('AuctionWindowSellControls.CalculateDeposit()');
    local cityId = RoR_CitySiege.GetCityFromPairing()
    --d('cityId: ' .. tostring(cityId))
    
    -- Get the city for the current pairing the player is in
    local pairingCityRating
    if cityId then pairingCityRating = GetCityRatingForCityId(cityId) end
    pairingCityRating = pairingCityRating or 1
    --d('pairingCityRating' .. tostring(pairingCityRating))

    -- 1* 10%, 2* 8%, 3* 6%, 4* 4%, 5* 2%
    local depositPrice = math.floor( (AuctionWindowSellControls.buyOutPrice / 100) * (12 - (2 * pairingCityRating)) )
    --d('vanilla depositPrice: ' .. tostring(depositPrice))

    if depositPrice < 1 then
		depositPrice = 1
    end
