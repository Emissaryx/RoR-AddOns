...
function Enemy.IntercomUI_IntercomDialog_Open ()
	...
	local function SetDisabledIfExists (name, disabled)
		if DoesWindowExist (name) then
			ButtonSetDisabledFlag (name, disabled)
		end
	end
	...
	then
		...
		SetDisabledIfExists ("EnemyIntercomDialogCreateButton", false)
		SetDisabledIfExists ("EnemyIntercomDialogInviteButton", false)
		SetDisabledIfExists ("EnemyIntercomDialogAskButton", true)
	else
	LabelSetText ("EnemyIntercomDialogMessage", L"You're currently not in any intercom channel")
	SetDisabledIfExists ("EnemyIntercomDialogCreateButton", true)
	SetDisabledIfExists ("EnemyIntercomDialogInviteButton", true)
	SetDisabledIfExists ("EnemyIntercomDialogAskButton", false)
...
function Enemy.IntercomUI_ConfigDialog_OnInitialize (section)
	...
	local lbl1 = config_dlg.section.windowName.."PrivateLabel"
	local lbl2 = config_dlg.section.windowName.."PrivateLabel2"
	if DoesWindowExist (lbl1) then
		LabelSetText (lbl1, L"WARNING!")
	end
	if DoesWindowExist (lbl2) then
		LabelSetText (lbl2, L"Usage of guild intercom requires relevant officer chat permissions!")
	end
end
...
function Enemy.IntercomUI_ConfigDialog_OnLoad (section)
	...
	-- Pick a valid, existing tab to press (skip 'Private' entirely)
	local candidates =
	{
		"EnemyConfigDialogSectionsIntercomAlliance",
		"EnemyConfigDialogSectionsIntercomGuild",
		"EnemyConfigDialogSectionsIntercomWarband",
		"EnemyConfigDialogSectionsIntercomRegion",
		"EnemyConfigDialogSectionsIntercomParty",
		"EnemyConfigDialogSectionsIntercomScenario",
		"EnemyConfigDialogSectionsIntercomScenarioParty"
	}

	for _, id in ipairs (candidates)
	do
		if DoesWindowExist (id)
		then
			ButtonSetPressedFlag (id, true)
			break
		end
	end

	-- Toggle state for 'Private' checkbox if it exists in this build/template
	local privButton = config_dlg.section.windowName.."Private"
	if DoesWindowExist (privButton) then
		ButtonSetPressedFlag (privButton, config_dlg.intercomPrivate)
	end
end
