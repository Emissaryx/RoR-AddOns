...
local HealDebuffsAbilityIds =
{
	In =
	{
		...
		{
			-- Zealot
			[3075]  = 50,  -- Changer's Touch
			[8613]  = 50,  -- Windblock
			-- Disciple of Khaine
			[9602]  = 50,  -- Curse of Khaine
			[3428]  = 50,  -- Curse of Khaine
			[32124] = 50,  -- Lacerate (Curse of Khaine tactic)
			-- Witch Elf
		...
		Order =
		{
			...
			-- Rune Priest
			[1633] = 50,  -- Rune of Nullification
			[3393] = 50,  -- Rune of Nullification
			-- Archmage
...
function EnemyEffectFilter:IsMatch (effect)
	...
	-- Helper: coerce any value (including wstring) into a Lua string, lowercased
	local function norm(v)
		if v == nil then return "" end
		if type(v) ~= "string" then v = tostring(v) end
		if v == nil then return "" end
		return string_lower(v)
	end

	-- Normalize EFFECT fields
	local _name = norm(effect._enemyName)
	local _text = norm(effect._enemyEffectText)

	-- Normalize FILTER fields
	local _filterName = self.name ~= nil and norm(self.name) or nil
	local _filterDesc = self.description ~= nil and norm(self.description) or nil

	if (_filterName and _filterName ~= "")
	then
		if (self.nameMatch == 1)
		then
			if (_name ~= _filterName) then return false end
		elseif (self.nameMatch == 2)
		then
			if (not string.find(_name, _filterName, 1, true)) then return false end
		elseif (self.nameMatch == 3)
		then
			if (not string.match(_name, _filterName)) then return false end
		end
	end

	if (_filterDesc and _filterDesc ~= "")
	then
		if (self.descriptionMatch == 1)
		then
			if (_text ~= _filterDesc) then return false end
		elseif (self.descriptionMatch == 2)
		then
			if (not string.find(_text, _filterDesc, 1, true)) then return false end
		elseif (self.descriptionMatch == 3)
		then
			if (not string.match(_text, _filterDesc)) then return false end
		end
	end
