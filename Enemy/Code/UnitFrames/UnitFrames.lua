...
local function UnitFrames_GetScoreboardSource ()
...
local function UnitFrames_BuildAltSpecDpsSet ()
	local playersData = UnitFrames_GetScoreboardSource ()
	...
	local set = {}
	local sigParts = {}
	for _, pdata in pairs (playersData)
	do
		local at = pdata and (pdata.archtype or pdata.archetype)
		if (at and at > 0 and pdata.name) then
			local key = NormalizeNameKey (pdata.name)
			if (key) then
				set[key] = true
				sigParts[#sigParts + 1] = key
			end
		end
	end
...
local function UnitFrames_BuildAltSpecIconMap (playersData, archtypeIcons)
	if (not playersData or not archtypeIcons) then
		return nil, nil
	end
	...
	do
		local at = pdata and (pdata.archtype or pdata.archetype)
		local iconId = at and at > 0 and archtypeIcons[at]
		if (iconId and pdata.name) then
			local key = NormalizeNameKey (pdata.name)
			if (key) then
				map[key] = iconId
				sigParts[#sigParts + 1] = key..":"..tostring (iconId)
			end
		end
	end
...
local function UnitFrames_RefreshScoreboardDrivenParts ()

	if (not g.frames or not Enemy.groups or not Enemy.groups.groups) then
		return
	end
	...
	do
		...
		local players_group = Enemy.groups.groups[group_index]

		if (frames_group and players_group)
		then
			for index = 1, 6
			do
				local frame = frames_group[index]
				local player = players_group[index]

				if (frame and frame:IsEnabled () and player) then
					frame:Update (player)
				end
			end
		end
...
local function UnitFrames_PollScoreboard ()
	local playersData, archtypeIcons = UnitFrames_GetScoreboardSource ()
	if (not playersData) then
		g.altSpecIconByKey = nil
		g._altSpecIconsDirty = false
		g._altSpecIconSignature = nil
		g.altSpecDpsSet = nil
		g._altSpecSignature = nil
		g._altSpecSetDirty = false
		return false
	end

	local now = Enemy.time or 0
	if (g._scoreboardNextPoll and now < g._scoreboardNextPoll) then
		return false
	end
	g._scoreboardNextPoll = now + SCOREBOARD_POLL_INTERVAL

	local inCombat = GameData and GameData.Player and GameData.Player.inCombat
	if (inCombat) then
		-- Skip all scoreboard scanning while in combat; just mark dirty so we rebuild once combat ends
		g._altSpecIconsDirty = true
		g._altSpecSetDirty = true
		return false
	end

	local count = 0
	for _ in pairs (playersData)
	do
		count = count + 1
	end

	local changed = false
	local needIconRefresh = false
	local needAltSpecRefresh = false
	if (g._scoreboardLastRef ~= playersData) then
		g._scoreboardLastRef = playersData
		changed = true
		needIconRefresh = true
		needAltSpecRefresh = true
	end

	if (g._scoreboardLastCount ~= count) then
		g._scoreboardLastCount = count
		changed = true
		needIconRefresh = true
		needAltSpecRefresh = true
	end

	local altSpecChanged = false
	local iconChanged = false

	if (needIconRefresh) then
		local map, sig = UnitFrames_BuildAltSpecIconMap (playersData, archtypeIcons)
		if (sig ~= g._altSpecIconSignature) then
			g.altSpecIconByKey = map
			g._altSpecIconSignature = sig
			iconChanged = true
		end
		g._altSpecIconsDirty = false
	end

	if (needAltSpecRefresh or g._altSpecSignature == nil) then
		local altSetNow, altSigNow = UnitFrames_BuildAltSpecDpsSet ()
		if (altSigNow ~= g._altSpecSignature) then
			g._altSpecSignature = altSigNow
			g.altSpecDpsSet = altSetNow
			altSpecChanged = true
		end
		g._altSpecSetDirty = false
	end

	if (g._altSpecIconsDirty and not inCombat) then
		-- scoreboard may not have changed since last poll, but we skipped rebuild due to combat
		local pd, ai = UnitFrames_GetScoreboardSource ()
		local map, sig = UnitFrames_BuildAltSpecIconMap (pd, ai)
		if (sig ~= g._altSpecIconSignature) then
			g.altSpecIconByKey = map
			g._altSpecIconSignature = sig
			iconChanged = true
	...
	if (g._altSpecSetDirty and not inCombat) then
		local altSetNow, altSigNow = UnitFrames_BuildAltSpecDpsSet ()
		if (altSigNow ~= g._altSpecSignature) then
			g._altSpecSignature = altSigNow
			g.altSpecDpsSet = altSetNow
	...
	if (changed or altSpecChanged or iconChanged) then
		if (g.settings.unitFramesSortingEnabled) then
			Enemy._UnitFramesRecalculatePosititionMatrix ()
		end
		UnitFrames_RefreshScoreboardDrivenParts ()
	end
...
function Enemy.UnitFramesInitialize ()
	...
	Enemy.AddTaskAction ("unitframes poll scoreboard", UnitFrames_PollScoreboard)
end
...
function Enemy._UnitFramesRecalculatePosititionMatrix (isTestMode)
	...
	local altSpecDpsSet = g.altSpecDpsSet

	local function GetSortedArchetype (frame)
		if (not frame or not frame.playerCareer or frame.playerCareer == 0) then
			return nil
		end

		local archetype = Enemy.careerArchetypes[frame.playerCareer]
		if (archetype and archetype ~= Enemy.Archetypes.Tank and altSpecDpsSet and frame.playerName) then
			local nameKey = NormalizeNameKey (frame.playerName)
		...
		then
			...
			then
				tsort (frames_indexes, function (index1, index2)
					...
					then
						local a = archetype_sorting[GetSortedArchetype (frame1)]
						local b = archetype_sorting[GetSortedArchetype (frame2)]
...
function Enemy.UnitFrames_OnGroupsUpdated (playerGroupChanged)
	...
	do
		...
		do
			...
			if (frame.windowName)
			then
				WindowSetDimensions(frame.windowName, g.settings.unitFramesSize[1], g.settings.unitFramesSize[2])
				WindowSetScale (frame.windowName, g.settings.unitFramesScale * global_scale)
			end
