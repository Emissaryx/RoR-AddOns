...
local function UpdateLabels()
	local offset = WarBoard_ClockSettings.Offset or 0
	local format = WarBoard_ClockSettings.Format or L"%02d:%02d"
	local r = WarBoard_ClockSettings.R or 255
	local g = WarBoard_ClockSettings.G or 255
	local b = WarBoard_ClockSettings.B or 255
	LabelSetText(COWindowName.."_lblRed",   StringToWString("R: "..r))
	LabelSetText(COWindowName.."_lblGreen", StringToWString("G: "..g))
	LabelSetText(COWindowName.."_lblBlue",  StringToWString("B: "..b))
	TextEditBoxSetText(COWindowName.."_edtStringFormat", format)
	TextEditBoxSetText(COWindowName.."_OffsetSeconds", towstring(tostring(offset)))
end
...
function WarBoard_ClockOptions.OnEnterKeyPressed()
	local val = TextEditBoxGetText(COWindowName.."_OffsetSeconds")
	if COWindowName == WindowGetParent(SystemData.ActiveWindow.name) then
		...
		WarBoard_ClockSettings.Offset = tonumber(WStringToString(val)) or 0
		SetClockWindowDims()
	end
end
