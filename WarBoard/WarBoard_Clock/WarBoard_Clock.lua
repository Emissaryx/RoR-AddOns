...
function WarBoard_Clock.CalcTime(total_seconds, offset)
	...
	-- Always normalize to 24-hour clock first
	local clock_hours_24 = total_hours % 24

	-- Determine AM/PM suffix without modifying the format string
	local suffix = L""
	if WarBoard_ClockSettings.AmPm then
		if clock_hours_24 < 12 then
			suffix = L" AM"
		else
			suffix = L" PM"
		end
	...
	-- Convert to display hours (12 or 24 mode)
	local display_hours = clock_hours_24 % WarBoard_ClockSettings.Hours
	if display_hours == 0 and WarBoard_ClockSettings.Hours == 12 then
		display_hours = 12
	end

	local format_string = WarBoard_ClockSettings.Format or L"%02d:%02d:%02d"

	return wstring_format(format_string, display_hours, clock_minutes, clock_seconds) .. suffix
end