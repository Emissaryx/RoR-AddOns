...
function SocialWindowTabFriends.OnMouseOverPlayerRow()
    ...
    if (SocialWindowTabFriends.playerListData[dataIndex].name == L"") then
        return
    end
    local friendData = SocialWindowTabFriends.playerListData[dataIndex]
    if friendData then
        ...
        if friendData.friendType == FRIENDTYPE.FRIENDED then
            -- description
            ...
            if friendData.isOnline then
                ...
                anchor = { Point="right", RelativeTo=SystemData.ActiveWindow.name, RelativePoint="left", XOffset=0, YOffset=44 }
            else
                Tooltips.SetTooltipColorDef( 1, 1, DefaultColor.MEDIUM_LIGHT_GRAY  )                
                Tooltips.SetTooltipText( iRow, 1,  friendData.onlineString )
                Tooltips.SetTooltipColorDef( iRow, 1, DefaultColor.MEDIUM_LIGHT_GRAY  )
                anchor = { Point="right", RelativeTo=SystemData.ActiveWindow.name, RelativePoint="left", XOffset=0, YOffset=14 } 
            end
            if friendData.isOnline then
                -- class
                iRow = iRow + 1
                ...
                -- guild
                iRow = iRow + 1
                if (friendData.guildName == nil) then
                    Tooltips.SetTooltipText( iRow, 1, GetString( StringTables.Default.LABEL_GUILD_NAME ))
                else
                    Tooltips.SetTooltipText( iRow, 1, GetString( StringTables.Default.LABEL_GUILD_NAME )..L" "..friendData.guildName )
                end
            end
        elseif friendData.friendType == FRIENDTYPE.SUGGESTED then
            ...
            anchor = { Point="right", RelativeTo=SystemData.ActiveWindow.name, RelativePoint="left", XOffset=0, YOffset=22} 
        end
...
function SocialWindowTabFriends.OnRButtonUpPlayerRow()
    ...
    if( SocialWindowTabFriends.SelectedFriendsName ~= L"" ) then
		local customItems = {}
        if SocialWindowTabFriends.playerListData[dataIndex].friendType == FRIENDTYPE.FRIENDED then
		    table.insert(customItems, PlayerMenuWindow.NewCustomItem(L"Add Description", SocialWindowTabFriends.OnAddDescription, false))
		    table.insert(customItems, PlayerMenuWindow.NewCustomItem(GetStringFromTable("SocialStrings", StringTables.Social.LABEL_SOCIAL_FRIENDS_BUTTON_REMOVEFRIEND), SocialWindowTabFriends.RemoveFriend, false))
        end
		PlayerMenuWindow.ShowMenu(SocialWindowTabFriends.SelectedFriendsName, 0, customItems)
    end
end

function SocialWindowTabFriends.OnAddDescription()
    WindowSetShowing( "AddFriendDescription", true )
	TextEditBoxSetText("AddFriendDescriptionText", SocialWindowTabFriends.GetDescription(SocialWindowTabFriends.SelectedFriendsName))
end
