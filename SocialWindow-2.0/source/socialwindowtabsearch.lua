...
function SocialWindowTabSearch.OnMouseOverPlayerRow()
    ...
    if (SocialWindowTabSearch.playerListData[dataIndex].name == L"" or SocialWindowTabSearch.playerListData[dataIndex].name == L"No Results") then
        return
    end
    ...
    -- PSYCHOXELL
    local SearchData = SocialWindowTabSearch.playerListData[dataIndex]
    if SearchData then
        Tooltips.CreateTextOnlyTooltip(SystemData.ActiveWindow.name)
        --name
        local anchor = {}
        local iRow = 1
        Tooltips.SetTooltipText(iRow, 1, SearchData.name)
        if (WStringsCompare(SearchData.location, GetStringFromTable( "SocialStrings", StringTables.Social.LABEL_SOCIAL_WINDOW_OFFLINE) ) == 0) then
            Tooltips.SetTooltipColorDef(1, 1, DefaultColor.MEDIUM_LIGHT_GRAY)  
            iRow = iRow + 1
            Tooltips.SetTooltipText(iRow, 1, SearchData.location)
            ...
            Tooltips.SetTooltipText(iRow, 1,  GetStringFormatFromTable("SocialStrings", StringTables.Social.TOOLTIP_BUDDY_LIST_LOCATION, {SearchData.location} ))
            -- class
            iRow = iRow + 1
            Tooltips.SetTooltipText( iRow, 1, GetStringFormatFromTable( "SocialStrings", StringTables.Social.TOOLTIP_BUDDY_LIST_CAREER, {SearchData.career} ) )
            -- rank
            iRow = iRow + 1
            Tooltips.SetTooltipText( iRow, 1, GetStringFormatFromTable( "SocialStrings", StringTables.Social.TOOLTIP_BUDDY_LIST_RANK, {SearchData.rankString} ) )
            -- guild
            iRow = iRow + 1
            Tooltips.SetTooltipText( iRow, 1, GetString( StringTables.Default.LABEL_GUILD_NAME )..L" "..SearchData.guildName )
